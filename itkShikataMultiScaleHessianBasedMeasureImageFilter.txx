 ...
 *
 *=========================================================================*/
#ifndef __itkShikataMultiScaleHessianBasedMeasureImageFilter_txx
#define __itkShikataMultiScaleHessianBasedMeasureImageFilter_txx
 ...
 * Constructor
 */
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage, THessianImage,
        THessianToMeasureImageFilter, TOutputImage>::ShikataMultiScaleHessianBasedMeasureImageFilter() {
    ...
    // set required input to be 2
    // 1: the original input intensity image
    // 2. the distance map of the mask
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
void ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::EnlargeOutputRequestedRegion(
        DataObject *output) {
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
typename ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::DataObjectPointer ShikataMultiScaleHessianBasedMeasureImageFilter<
        TInputImage, THessianImage, THessianToMeasureImageFilter, TOutputImage>::MakeOutput(
        unsigned int idx) {
    if (idx == 1) {
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
void ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::AllocateUpdateBuffer() {
    ...
    if (m_NonNegativeHessianBasedMeasure) {
        ...
        std::cout << "non pos min" << itk::NumericTraits<BufferValueType>::NonpositiveMin() << std::endl;
    ...
    it.GoToBegin();
    std::cout << "initial min in m_UpdateBuffer" << it.Get() << std::endl;
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
void ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::GenerateData() {
    ...
    if (m_GenerateScalesOutput) {
        typename ScalesImageType::Pointer
                scalesImage =
                        dynamic_cast<ScalesImageType *> (this->ProcessObject::GetOutput(
                                1));
    ...
    if (m_GenerateHessianOutput) {
        typename HessianImageType::Pointer
                hessianImage =
                        dynamic_cast<HessianImageType *> (this->ProcessObject::GetOutput(
                                2));
    ...
    if (m_NumberOfSigmaSteps > 0) {
        progress->RegisterInternalFilter(this->m_HessianFilter,
                .5 / m_NumberOfSigmaSteps);
        progress->RegisterInternalFilter(this->m_HessianToMeasureFilter,
                .5 / m_NumberOfSigmaSteps);
    }

    //    double sigma = m_SigmaMinimum;
    double sigma, distanceThreshold;
    //int scaleLevel = 1;

    unsigned int number_of_levels = m_SigmaList.size();

    if (number_of_levels != m_DistanceThresholdList.size()) {
        std::cout
                << "DistanceThresholdList and SigmaList do not have same size!!!"
                << std::endl;
        exit(-1);
    }

    unsigned int scaleLevel = 0;
    while (scaleLevel < number_of_levels) {

        sigma = m_SigmaList[scaleLevel];
        distanceThreshold = m_DistanceThresholdList[scaleLevel];
        ...
        m_HessianToMeasureFilter->SetInputHessianImage(
                m_HessianFilter->GetOutput());

        m_HessianToMeasureFilter->SetInputScalarImage(input);

        m_HessianToMeasureFilter->SetSigmaF(sigma);

        std::cout << "set hessian and scalar for sigma=" << sigma
                << " , distanceThrehold=" << distanceThreshold << std::endl;

        m_HessianToMeasureFilter->Update();

        this->UpdateMaximumResponse(sigma, distanceThreshold);
    ...
    ImageRegionIterator<UpdateBufferType> it(m_UpdateBuffer, outputRegion);
    it.GoToBegin();
    std::cout << "min in m_UpdateBuffer" << it.Get() << std::endl;
    ...
    while (!oit.IsAtEnd()) {
        ...
        if (b){
            std::cout << "min in m_UpdateBuffer" << it.Get() << std::endl;
            std::cout << "min in output" << oit.Get() << std::endl;
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
void ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::UpdateMaximumResponse(
        double sigma, double distanceThreshold) {
    ...
    typename DistanceImageType::Pointer distanceImage =
            static_cast<DistanceImageType *> (this->ProcessObject::GetInput(1));
    ImageRegionIterator<DistanceImageType> idit(distanceImage, outputRegion);
    // std::cout << "distanceImage: " << distanceImage << std::endl;
    ...
    if (m_GenerateHessianOutput) {
        ohit = ImageRegionIterator<HessianImageType> (hessianImage,
                outputRegion);
        ohit.GoToBegin();
    }
    ...
    ImageRegionIterator<HessianToMeasureOutputImageType> it(
    ...
    float min_distance = itk::NumericTraits<float>::max();
    float max_distance = itk::NumericTraits<float>::NonpositiveMin();


    while (!oit.IsAtEnd()) {

        min_distance = (min_distance < idit.Value()) ? min_distance : (idit.Value());
        max_distance = (max_distance > idit.Value()) ? max_distance : (idit.Value());

        if (idit.Value() >= distanceThreshold) {
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
double ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::ComputeSigmaValue(
        int scaleLevel) {
    double sigmaValue;
    ...
    case Self::LogarithmicSigmaSteps: {
        const double stepSize = vnl_math_max(
                1e-10,
                (vcl_log(m_SigmaMaximum) - vcl_log(m_SigmaMinimum))
                        / (m_NumberOfSigmaSteps - 1));
        sigmaValue = vcl_exp(vcl_log(m_SigmaMinimum) + stepSize * scaleLevel);
        break;
    }
    default:
        throw ExceptionObject(__FILE__, __LINE__, "Invalid SigmaStepMethod.",
                ITK_LOCATION);
        break;
    }
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
void ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::SetSigmaStepMethodToEquispaced() {
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
void ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::SetSigmaStepMethodToLogarithmic() {
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
const typename ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::HessianImageType *
ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage, THessianImage,
        THessianToMeasureImageFilter, TOutputImage>::GetHessianOutput() const {
    return static_cast<const HessianImageType *> (this->ProcessObject::GetOutput(
            2));
}
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
const typename ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::ScalesImageType *
ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage, THessianImage,
        THessianToMeasureImageFilter, TOutputImage>::GetScalesOutput() const {
    return static_cast<const ScalesImageType *> (this->ProcessObject::GetOutput(
            1));
}
...
template<typename TInputImage, typename THessianImage,
        typename THessianToMeasureImageFilter, typename TOutputImage>
void ShikataMultiScaleHessianBasedMeasureImageFilter<TInputImage,
        THessianImage, THessianToMeasureImageFilter, TOutputImage>::PrintSelf(
    ...
    os << indent << "NonNegativeHessianBasedMeasure:  "
            << m_NonNegativeHessianBasedMeasure << std::endl;
